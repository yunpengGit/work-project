<template>
  <div>
    <div v-if="companyData">
      <Breadcrumb :baseBreadcrumbData="['往期异常订单']" style="margin: 8px 0 16px;padding-bottom: 16px;"/>
        ...
        >
          ...
          <div class="btn">
            ...
            <span v-else>
              ...
              <a-button type="primary" @click="updateAndCorrectOrderFn">
...
<script>
import { update, getSingleOrderByIdInSupplier, deleteById, deleteByIds } from '@/api/order/success.js'
import { getSupplierAirTicketExceptionOrderListPageByParams, tmcCorrectOrder, updateAndCorrectOrder } from '@/api/order/pastAbnormal.js'
...
export default {
  ...
  methods: {
    ...
    updateAndCorrectOrderFn() {
      const data = cloneDeep(this.drawerFormData)
      data.departureTime = data.departureTime.format('YYYY-MM-DD HH:mm:ss')
      data.preDate = data.preDate.format('YYYY-MM-DD HH:mm:ss')
      data.arrivingTime = data.arrivingTime.format('YYYY-MM-DD HH:mm:ss')
      data.dateOfDraft = data.dateOfDraft.format('YYYY-MM-DD HH:mm:ss')
      updateAndCorrectOrder(data).then((res) => {
        this.$message.success(res.resultStr)
        this.visible = false
        this.$refs.table.refreshFn()
      })
