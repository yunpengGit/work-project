        ...
        <el-dialog :title="title" :width="width" top="12%" v-if="dialogVisible" :visible.sync="dialogVisible"
            ...
            <div v-show="!shangchuanwancheng">
                <div style="width: 100%;padding-bottom:10px">
                    ...
                    <el-link :href="publicImagePath+'importTemplate/票证/车证导入模板.xlsx'" :underline="false" type="primary"
                        style="float: right">
...
<script>
import { getImportVehicleCardResult } from '@/api/card'
...
export default {
    ...
    data () {
        return {
            ...
            uploadUrl: process.env.VUE_APP_BASE_API + '/synergy/files/import/card/vehicle',
            uploadData: {
                uuid: '',
    ...
    methods: {
        ...
        partMethod () {
            var that = this;
            setTimeout(function () {
                that.getImportVehicleCardResult()
            }, 1000);
        },
        closeLoading () {
            if (this.load !== null) {
        ...
        getImportVehicleCardResult () {
            getImportVehicleCardResult({ 'uuid': this.uploadData.uuid, 'conferenceId': getConferenceId() }).then(res => {
