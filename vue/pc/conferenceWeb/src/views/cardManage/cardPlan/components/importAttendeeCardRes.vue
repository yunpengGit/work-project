        ...
        <el-dialog :title="title" :width="width" top="12%" v-if="dialogVisible" :visible.sync="dialogVisible"
            ...
            <div v-show="!shangchuanwancheng">
                <div style="width: 100%;padding-bottom:10px">
                    ...
                    <el-link :href="publicImagePath+'importTemplate/票证/嘉宾证导入模板.xlsx'" :underline="false" type="primary"
                        style="float: right">
...
<script>
import { getImportAttendeeCardResult } from '@/api/card'
...
export default {
    ...
    data () {
        return {
            ...
            uploadUrl: process.env.VUE_APP_BASE_API + '/synergy/files/import/card/attendee',
            uploadData: {
                uuid: '',
    ...
    methods: {
        ...
        partMethod () {
            var that = this;
            setTimeout(function () {
                that.getImportAttendeeCardResult()
            }, 1000);
        },
        closeLoading () {
            if (this.load !== null) {
        ...
        getImportAttendeeCardResult () {
            getImportAttendeeCardResult({ 'uuid': this.uploadData.uuid, 'conferenceId': getConferenceId() }).then(res => {
