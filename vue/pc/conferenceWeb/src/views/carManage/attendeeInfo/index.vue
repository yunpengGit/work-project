    ...
    <div style="padding-left:8px;padding-right:8px;margin-top:8px">
        ...
        <div style="background-color: white;padding-left:10px;padding-top:10px;padding-right: 10px">
            ...
            <div style="float: right;padding-bottom:10px">
                ...
                <el-button v-permission="buttons.CarAttendeeInfo.export" @click="exportExcel">导出查询结果</el-button>
            ...
            <el-table
                ...
                style="width: 100%"
                :height="tableHeight+ (this.$store.state.menu.buttons.indexOf(Global.buttons.CarAttendeeInfo.export) !== -1
                    ? 0: 32)"
            ...
            >
                ...
                <el-table-column prop="name" label="姓名" :min-width="155">
                    <template slot-scope="scope">
                        <div style="height:32px">
                            <AttendeeInfomation :accompany-flag="2" :accompany-totips="scope.row.accompanyTotips" :photo-url="scope.row.fixAttributeCommonVO.photoUrl" :attendee-name="scope.row.fixAttributeCommonVO.attendeeName" :attendee-role-flag="scope.row.attendeeRoleFlag" :honour-flag="scope.row.fixAttributeCommonVO.honourFlag" :sex="scope.row.fixAttributeCommonVO.sex" />
                        </div>
                        <!-- <el-tooltip
                ...
                <el-table-column label="职务" :min-width="125">
                    <template slot-scope="scope">
                        <div class="content1">
                ...
                <el-table-column label="手机" :min-width="125">
                    <template slot-scope="scope">
                ...
                <el-table-column label="单位简称/全称" :min-width="125">
                    <template slot-scope="scope">
                        <div class="content1">
                ...
                <el-table-column label="参会状态" :min-width="125">
                    <template slot-scope="scope">
                        {{ scope.row.meetingStatusStr }}
                    </template>
                </el-table-column>
                <!-- <el-table-column label="用车信息"  >
                    <template slot-scope="scope">
                        <el-popover
                            v-if="scope.row.vehicleTaskList !=null && scope.row.vehicleTaskList.length>0 "
                            placement="bottom-start"
                            width="250px"
                            trigger="hover"
                        >
                            <div v-for="vehicleTaskList in scope.row.vehicleTaskList" :key="vehicleTaskList.id">
                                {{ vehicleTaskList.startTime.slice(5,16) }} ~
                                {{ vehicleTaskList.endTime.slice(5,16) }} &nbsp;&nbsp;
                                {{ vehicleTaskList.startAddress }}
                                ~{{ vehicleTaskList.endAddress }}&nbsp;&nbsp;
                                <span style="color:#00BFFF">{{ vehicleTaskList.vehicle.platNum }} </span>&nbsp;&nbsp;
                                <span style="color:#00BFFF">{{ vehicleTaskList.vehicle.vehicleTypeVO.name }} </span><br>
                                &nbsp;&nbsp;&nbsp;&nbsp; 司机： {{ vehicleTaskList.driverVO.name }}&nbsp;&nbsp;
                                {{ vehicleTaskList.driverVO.phone }}<br>
                                &nbsp;&nbsp; &nbsp;&nbsp;联系人： {{ vehicleTaskList.contactPerson }}&nbsp;&nbsp;
                                {{ vehicleTaskList.phone }}
                            ...
                            <div
                                ...
                                class="content"
                            >
                                {{ scope.row.vehicleTaskList[0].startTime.slice(5,16) }} ~
                                {{ scope.row.vehicleTaskList[0].endTime.slice(5,16) }}
                                {{ scope.row.vehicleTaskList[0].startAddress }}
                                ~{{ scope.row.vehicleTaskList[0].endAddress }}....
                ...
                <el-table-column label="操作" :min-width="155">
                    <template slot-scope="scope">
                        ...
                        <el-button size="mini" @click="chakanxingcheng(scope.row.id)">查看用车</el-button>
                    </template>
                </el-table-column>
...
<script>
import { getAttendeeInfoList, deleteVehicleTaskAttendee } from '@/api/car'
...
import { exportVehiclTaskAttendeeQuery } from '@/api/synergyFiles'
export default {
    name: '',
    components: {
        commonSearch,
    ...
    methods: {
        exportExcel() {
            ...
            exportVehiclTaskAttendeeQuery(exportParams).then(res => {
        ...
        chakanxingcheng(attendeeId) {
            ...
            this.defaultCheck = 'useVehicleFlag'
        },
