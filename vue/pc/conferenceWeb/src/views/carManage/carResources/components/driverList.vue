    ...
    <div>
            ...
            <div style="background-color: white;padding-left:10px;padding-top: 15px">
                <el-form ref="form" :inline="true" :model="form">
                    ...
                    <el-form-item label="公司名称" prop="companyNameLike">
                        <el-input v-model.trim="form.companyNameLike" size="small" style="width: 250px" />
                    </el-form-item>
        ...
        <div style="background-color: white;padding-left:10px;padding-top:10px;padding-right: 10px">
            <div style="float: left; padding-bottom:10px">
                <el-button v-permission="buttons.Driver.batchDelete" type="primary" @click="delSelectedWithConfirm">批量删除
                </el-button>
                <el-button v-permission="buttons.Driver.importShare" type="primary" @click="importRes">
                    导入共享资源</el-button>
                <el-button v-permission="buttons.Driver.addToShare" type="primary" @click="addToRes">
                    添加至共享资源</el-button>
            </div>
            <div style="float: right;padding-bottom:10px">
                <el-button v-permission="buttons.Driver.insert" type="primary" @click="handleEdit">添加司机</el-button>
                <el-button v-permission="buttons.Driver.import" @click="importDriver">导入司机信息</el-button>
                <el-button v-permission="buttons.Driver.export" @click="exportDriver">导出查询结果</el-button>
            </div>
            <el-table
                ref="table"
                :data="driverList"
                :height="tableHeight+ (this.$store.state.menu.buttons.indexOf(Global.buttons.Driver.batchDelete) !== -1 ||
                    this.$store.state.menu.buttons.indexOf(Global.buttons.Driver.insert) !== -1 ||
                    this.$store.state.menu.buttons.indexOf(Global.buttons.Driver.import) !== -1 ||
                    this.$store.state.menu.buttons.indexOf(Global.buttons.Driver.importShare) !== -1 ||
                    this.$store.state.menu.buttons.indexOf(Global.buttons.Driver.addToShare) !== -1 ||
                    this.$store.state.menu.buttons.indexOf(Global.buttons.Driver.export) !== -1 ? 0: 32)"
                style="width: 100%"
            ...
            >
                ...
                <el-table-column prop="vehicleCompanyVO.name" label="公司名称" />
                <el-table-column label="操作">
                    <template slot-scope="scope">
                        <el-button
                            v-permission="buttons.Driver.edit"
                            size="mini"
                            @click="handleEdit(scope.$index, scope.row)"
                        >编辑</el-button>
                        <el-button
                            v-permission="buttons.Driver.delete"
                            size="mini"
                            type="danger"
        ...
        <editDriver ref="editDriver" @getListInfo="getListInfo(paramData)" />
        <importDriverRes :dialog-visible="importDialogVisible" @closeDialog="closeImportDialog" />
    </div>
</template>

<script>

import { getDriverList, deleteDriver, shareDriver, downDriver, exportDriver } from '@/api/car'
import editDriver from './editDriver'
import { getTableHeight, formatPhone } from '@/utils/index'
import { getConferenceId } from '@/utils/conference-auth'
import importDriverRes from './importDriverRes'
import { showLoading, closeLoading } from '@/utils/loading'

export default {
    name: 'DriverList',
    components: { editDriver, importDriverRes },
    data() {
        return {
    ...
    methods: {
        exportDriver() {
            ...
            exportDriver(tempData).then(res => {
        ...
        importRes() {
            downDriver('conferenceId=' + getConferenceId()).then(res => {
                if (res.success) {
        ...
        addToRes() {
            shareDriver('conferenceId=' + getConferenceId()).then(res => {
                if (res.success) {
        ...
        delSelected() {
            ...
            console.log(ids)
            deleteDriver(this.$qs.stringify(params)).then(response => {
                if (response.success) {
                    this.$message('删除成功')
        ...
        handleEdit(index, row) {
            if (row) {
                this.$refs['editDriver'].showEdit(row)
            } else {
                this.$refs['editDriver'].showEdit()
            }
        },
        ...
        getListInfo(param) {
            getDriverList(param).then(response => {
                if (response.success) {
                    console.log('driverList' + response.result.list)
