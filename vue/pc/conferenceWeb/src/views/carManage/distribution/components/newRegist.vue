    ...
    <div>
        <div style="padding-left:8px;padding-right:8px;margin-top:0px">
            <div style="padding-left:10px;padding-top: 0px">
                <pick-up-search ref="commonSearch" @onSubmit="onSubmit" @changeTableHeight="changeTableHeight" />
            </div>
        </div>
        ...
        <div style="background-color: white;padding-left:10px;padding-top:10px;padding-right: 10px">
            <div style="float: left; padding-bottom:10px">
                <el-button v-permission="buttons.PickUp.batchDistribute" type="primary" @click="handleDialog">批量分配用车
                </el-button>
            ...
            <div style="float: right;padding-bottom:10px">
                ...
                <el-button v-permission="buttons.PickUp.export" @click="exportExcel">导出查询结果</el-button>
                <!-- <el-button v-permission="buttons.PickUp.export">导出查询结果</el-button> 20210527biaoji -->
            ...
            <el-table
                ...
                :height="tableHeight+ (this.$store.state.menu.buttons.indexOf(Global.buttons.PickUp.export) !== -1 ||
                    this.$store.state.menu.buttons.indexOf(Global.buttons.PickUp.batchDistribute) !== -1
                    ? 0: 32)"
                style="width: 100%"
            ...
            >
                ...
                <el-table-column label="到达时间" show-overflow-tooltip :min-width="120">
                    <template slot-scope="scope">
                        <span v-if="scope.row.attendee.fixAttributeCommonVO.arriveTime">
                            {{ scope.row.attendee.fixAttributeCommonVO.arriveTime.slice(0,16) }}
                        </span>
                        <span v-else>
                            -
                        </span>
                ...
                <el-table-column label="到达地点" show-overflow-tooltip :min-width="90">
                    <template slot-scope="scope">
                        {{ formatTableCellData(scope.row.attendee.fixAttributeCommonVO.arriveStation) }}
                    </template>
                </el-table-column>
                <el-table-column label="班次/车次" show-overflow-tooltip :min-width="90">
                    <template slot-scope="scope">
                        {{ formatTableCellData(scope.row.attendee.fixAttributeCommonVO.arriveFlight) }}
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="260px" fixed="right">
                    <template slot-scope="scope">
                        <div>
                            ...
                            <el-button
                                v-if="scope.row.attendeeTask !==null && scope.row.attendeeTask.result !==100 && scope.row.attendeeTask.result !==110 "
                                v-permission="buttons.PickUp.ignore"
                                size="mini"
                            ...
                            <el-button
                                v-permission="buttons.PickUp.distribute"
                                size="mini"
        ...
        <addTask :dialog-visible="addTaskDialogVisible" flag="pickUp" @closeDialog="closeAddTask" />
    </div>
</template>
<script>
import taskDialog from './taskDialog'
import attendeeTaskDialog from './attendeeTaskDialog'
import { getPickUpTaskAttendeeList, updateTaskState, ignoreTask, getPickUpAttendeeList } from '@/api/car'
import { getTableHeight, formatPhone } from '@/utils/index'
import { getConferenceId } from '@/utils/conference-auth'
import { getOssBaseUrl } from '@/utils/x-sso-baseurl'
import PickUpSearch from './pickUpSearch.vue'
import addTask from './addTask'
import { showLoading, closeLoading } from '@/utils/loading'
import { exportVehiclTaskAttendee } from '@/api/synergyFiles'
export default {
    name: 'NewRegist',
    components: { taskDialog, PickUpSearch, attendeeTaskDialog, addTask },
    data() {
        return {
    ...
    methods: {
        ...
        handleAttendeeTaskDialog(index, row) {
            this.$refs['attendeeTaskDialog'].showDialog(row, 1) // type=1  接站用车
        },
        ...
        handleDialog(index, row) {
            if (row) {
                var list = []
                list.push(row)
                this.$refs['taskDialog'].showDialog(list, 'pickUp')
            } else {
                var selectedList = this.$refs.table.selection
                if (selectedList.length > 0) {
                    this.$refs['taskDialog'].showDialog(selectedList, 'pickUp')
                } else {
        ...
        getListInfo(param) {
            getPickUpAttendeeList(param).then(response => {
                if (response.success) {
