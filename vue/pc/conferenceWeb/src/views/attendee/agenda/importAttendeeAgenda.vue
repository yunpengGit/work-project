...
import { getImportResult } from '@/api/agenda'
import {
    showTextLoading,
    closeLoading
} from '@/utils/loading'
...
export default {
    ...
    data() {
        return {
            ...
            uploadUrl: process.env.VUE_APP_BASE_API + '/excel/importExcel',
            uploadData: {
    ...
    methods: {
        xiazaimuban() {
            ...
            downloadFile({ filePath: 'importTemplate/议程导入模板.xlsx' }).then(res => {
                if (decodeURI(res.headers['filename']) == null || decodeURI(res.headers['filename']) === 'undefined') {
                    this.$message('文件不存在')
                } else {
                    const blob = new Blob([res.data], { type: 'application/octet-stream' })
                    if (window.navigator && window.navigator.msSaveOrOpenBlob) {
                        navigator.msSaveBlob(blob, '议程导入模板.xlsx')
                    } else {
                        const link = document.createElement('a')
                        link.style.display = 'none'
                        link.href = URL.createObjectURL(blob)
                        link.setAttribute('download', '议程导入模板.xlsx')
        ...
        beforeUpload(file) {
            ...
            this.uploadData.agendaId = this.agendaId
