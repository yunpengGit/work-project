        ...
        >
            ...
            <div style="background-color: white;padding-left:10px;padding-top:10px;padding-right: 10px">
                <div style="float: left; text-center: left;margin-left:10px;padding-bottom:10px;">
                    <el-button v-permission="Global.buttons.AttendeeTag.deleteAttendee" @click="deleteBatch">批量删除</el-button>
                </div>
                <!-- <div style="float: right; text-center: right;padding-bottom:10px;">
                    <el-button v-permission="Global.buttons.AttendeeTag.addAttendee" type="primary" @click="add">添加</el-button>
                </div> -->
                <el-table
                    ref="table"
                    :data="tableData"
                    :height="(dakaichaxunClass === 0 ? (tableHeight - 52) : tableHeight) + (this.$store.state.menu.buttons.indexOf(Global.buttons.AttendeeTag.addAttendee) !== -1 ||
                        this.$store.state.menu.buttons.indexOf(Global.buttons.AttendeeTag.deleteAttendee) !== -1 ? 0: 32)"
                    style="width: 100%;"
...
import { insertTagAttendee, deleteByAttendeeIdsAndTagId } from '@/api/attendeeTag'
...
export default {
    ...
    methods: {
        ...
        deleteSubmit(param) {
            this.$confirm('请确认是否删除标签下人员信息？', '', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                deleteByAttendeeIdsAndTagId({ 'tagId': this.tagId, 'attendeeIds': param }).then(res => {
                    if (res.success) {
