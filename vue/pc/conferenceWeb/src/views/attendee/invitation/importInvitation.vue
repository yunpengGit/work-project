        ...
        >
            ...
            <div v-show="shangchuanwancheng">
                <div style="width: 100%;padding-bottom:10px;height:100px;margin-top:30px;">
                    ...
                    <el-link v-if="errorNumber > 0 && errorFilePath !== null && errorFilePath !== '' && errorFilePath !== '0'" :underline="false" type="primary" style="float: right" @click="downloadError">
                        <i class="el-icon-download" />
                        下载错误文件
                    </el-link>
...
import { getImportResult } from '@/api/invitation'
import {
    showTextLoading,
    closeLoading
} from '@/utils/loading'
...
export default {
    ...
    data() {
        return {
            ...
            uploadUrl: process.env.VUE_APP_BASE_API + '/conferenceInviteUser/importExcel',
            uploadData: {
    ...
    methods: {
        xiazaimuban() {
            ...
            downloadFile({ filePath: 'importTemplate/会议邀请导入模板.xlsx' }).then(res => {
                if (decodeURI(res.headers['filename']) == null || decodeURI(res.headers['filename']) === 'undefined') {
                    this.$message('文件不存在')
                } else {
                    const blob = new Blob([res.data], { type: 'application/octet-stream' })
                    if (window.navigator && window.navigator.msSaveOrOpenBlob) {
                        navigator.msSaveBlob(blob, '会议邀请导入模板.xlsx')
                    } else {
                        const link = document.createElement('a')
                        link.style.display = 'none'
                        link.href = URL.createObjectURL(blob)
                        link.setAttribute('download', '会议邀请导入模板.xlsx')
        ...
        getImportResult() {
            getImportResult(this.uploadData).then(res => {
                if (res.success) {
                    if (res.result.state === 1 || (res.result.errorNumber > 0 && res.result.errorFile === '0')) {
