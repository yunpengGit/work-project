        ...
        >
            ...
            <div v-show="!shangchuanwancheng">
                <div style="width: 100%;padding-bottom:10px">
                    ...
                    <!-- <el-link :href="publicImagePath + 'importTemplate/会议邀请导入模板.xls'" :underline="false" type="primary" style="float: right">
                        <i class="el-icon-download" />
                        下载模板
                    </el-link> -->
            ...
            <div v-show="shangchuanwancheng">
                <div style="width: 100%;padding-bottom:10px;height:100px;margin-top:30px;">
                    导入完成。商网用户成功{{ sheet1SucceedNumber }}条，失败{{ sheet1ErrorNumber }}条。
                    非商网用户成功{{ sheet2SucceedNumber }}条，失败{{ sheet2ErrorNumber }}条。
                    <el-link v-if="(sheet1ErrorNumber > 0 || sheet2ErrorNumber > 0) && errorFilePath !== null && errorFilePath !== '' && errorFilePath !== '0'" :underline="false" type="primary" style="float: right" @click="downloadError">
                        <i class="el-icon-download" />
                        下载错误文件
                    </el-link>
...
import { getImportAttendeeModel, getImportResult } from '@/api/attendee'
import {
    showTextLoading,
    closeLoading
} from '@/utils/loading'
...
export default {
    ...
    data() {
        return {
            ...
            fileList: [],
            sheet1SucceedNumber: 0,
            sheet1ErrorNumber: 0,
            sheet2SucceedNumber: 0,
            sheet2ErrorNumber: 0,
            ...
            uploadUrl: process.env.VUE_APP_BASE_API + '/excel/attendee/importExcel',
            uploadData: {
    ...
    watch: {
        dialogVisible() {
            ...
            this.sheet1SucceedNumber = 0
            this.sheet1ErrorNumber = 0
            this.sheet2SucceedNumber = 0
            this.sheet2ErrorNumber = 0
        }
    ...
    methods: {
        xiazaimuban() {
            ...
            getImportAttendeeModel({ conferenceId: getConferenceId() }).then(res => {
        ...
        getImportResult() {
            getImportResult(this.uploadData).then(res => {
                if (res.success) {
                    if ((res.result.sheet1State === 1 || res.result.sheet2State === 1) || ((res.result.sheet1ErrorNumber > 0 || res.result.sheet2ErrorNumber > 0) && res.result.errorFile === '0')) {
                        this.importResultText = '正在导入，商网用户成功' + res.result.sheet1SucceedNumber + '条，失败' + res.result.sheet1ErrorNumber + '条，非商网用户成功' + res.result.sheet2SucceedNumber + '条，失败' + res.result.sheet2ErrorNumber + '条。'
                        ...
                        this.sheet1SucceedNumber = res.result.sheet1SucceedNumber
                        this.sheet1ErrorNumber = res.result.sheet1ErrorNumber
                        this.sheet2SucceedNumber = res.result.sheet2SucceedNumber
                        this.sheet2ErrorNumber = res.result.sheet2ErrorNumber
