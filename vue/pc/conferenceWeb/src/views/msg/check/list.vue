    ...
    <div class="clearfix">
      ...
      >
        <el-form :inline="true" :model="pageInfo">
          ...
          <el-form-item label="发送状态" v-if="!showinquire">
          ...
          <el-form-item label="发送时间" v-if="!showinquire">
            <el-date-picker
              v-model="inputTimes"
...
export default {
  ...
  created() {
    if (this.$route.query.from && this.$route.query.from === "hand") {
      this.pageInfo.templatelogId = this.$route.query.templatelogId;
      this.verifyStatus=this.$route.query.verifyStatus
      if(!this.$route.query.hasOwnProperty('isinquire')){
        this.showinquire=true
      }
      if(this.$route.query.hasOwnProperty('isinquire')&&this.verifyStatus!==1){
        this.isinquire=false
      }
    } else {
      this.pageInfo.themeCode = this.$route.query.code;
    }
    this.getThemeCodes();
    this.search();
    this.getTagList()
  ...
  methods: {
    ...
    getThemeCodes() {
      getDicList({ code: "themeCode" }).then((response) => {
        ...
        this.themeCodes.forEach((item) => {
          if (item.valCode === this.$route.query.code) {
    ...
    resetSearch() {
      this.inputTimes = [];
      this.pageInfo = {
        // themeCode: this.$route.query.code,
        pageNum: 1,
        pageSize: 10,
