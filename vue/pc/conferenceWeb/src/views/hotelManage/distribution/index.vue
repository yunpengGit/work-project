    ...
    <div style="padding-left:8px; padding-top:8px;padding-right:8px;">
        ...
        <div style="background-color: white;padding-left:10px;padding-top:10px;padding-right: 10px">
            <div style="float: left; padding-bottom:10px">
                <el-button
                    v-permission="buttons.HotelDistribution.batchDistribution"
                    type="primary"
                    @click="handleDialog"
                >批量分配</el-button>
            ...
            <div style="float: right;padding-bottom:10px">
                <el-button v-permission="buttons.HotelDistribution.export" @click="exportExcel">导出查询结果</el-button>
            ...
            <el-table
                ...
                :height="tableHeight+ (this.$store.state.menu.buttons.indexOf(Global.buttons.HotelDistribution.batchDistribution) !== -1
                    ? 0: 32)"
                style="width: 100%"
            ...
            >
                ...
                <el-table-column label="性别" :min-width="55" show-overflow-tooltip>
                    <template slot-scope="scope">
                        <span v-show="scope.row.attendee.fixAttributeCommonVO.sex === '1'">男</span>
                        <span v-show="scope.row.attendee.fixAttributeCommonVO.sex === '2'">女</span>
                ...
                <el-table-column label="标签">
                    <template slot-scope="scope">
                        ...
                        <el-tag
                            v-for="(tagVO) in scope.row.attendee.tagVOs"
                            :key="tagVO.id"
                ...
                <el-table-column label="返程时间" :min-width="120">
                    <template slot-scope="scope">
                ...
                <el-table-column label="操作" fixed="right" width="250">
                    <template slot-scope="scope">
                        <el-button size="mini" @click="chakanxingcheng(scope.row.attendee.id)">查看用房</el-button>
                        <el-button
                            v-if="scope.row.attendeeTask !==null && scope.row.attendeeTask.result !==100 && scope.row.attendeeTask.result !==110 "
                            v-permission="buttons.HotelDistribution.ignore"
                            size="mini"
                            @click="handleIgnoreWithConfirm(scope.$index, scope.row)"
                        >忽略
                        </el-button>
                        <el-button
                            v-permission="buttons.HotelDistribution.distribution"
                            size="mini"
...
import { getSynergyAttendeeTask } from '@/api/hotel'
...
import { exportRoomTaskAttendee } from '@/api/synergyFiles'
export default {
    name: 'NewRegist',
    ...
    data() {
        return {
            ...
            paramData: {
                ...
                //  typeEqual: 52, // 50：用车资源分配，51：用餐资源分配，52：住宿资源分配，53：制证资源分配’
                taskResult: 101, // 分配状态默认未分配
    ...
    methods: {
        exportExcel() {
            ...
            exportRoomTaskAttendee(exportParams).then(res => {
        ...
        getListInfo(param) {
            getSynergyAttendeeTask(param).then(response => {
                if (response.success) {
