...
```javascript
import { MessageBox } from 'golden-pc-ui';
```
...
设置 size 属性可以渲染不同尺寸的Message Box控件。

:::demo `size`的值 mini / small /medium 分别代表在14px / 16px / 18px下渲染出的的Message Box控件。
```html
<el-row>
  <el-button type="text" @click="open('mini')">点击打开 Message Box的 mini 版</el-button>
</el-row>
<el-row>
  <el-button type="text" @click="open('small')">点击打开 Message Box的 small 版</el-button>
</el-row>
<el-row>
  <el-button type="text" @click="open('medium')">点击打开 Message Box的 medium 版</el-button>
</el-row>
...
| 参数      | 说明          | 类型      | 可选值                           | 默认值  |
|---------- |-------------- |---------- |--------------------------------  |-------- |
| size | 用于控制Message Box组件内的字体、Icon尺寸大小 | string | medium / small / mini | — |
