...
```javascript
import { Loading } from 'golden-pc-ui';
```
在需要调用时：
```javascript
...
设置 size 属性可以渲染不同尺寸的Loading控件。

:::demo `size`的值 mini / small /medium 分别代表在14px / 16px / 18px下渲染出的的Loading控件。
```html
<el-row>
  <el-table
    v-loading="loading"
    element-loading-size="mini"
    element-loading-text="拼命加载中"
    element-loading-spinner="el-icon-loading"
    element-loading-background="rgba(0, 0, 0, 0.8)"
    :data="tableData"
...
| 参数      | 说明          | 类型      | 可选值                           | 默认值  |
|---------- |-------------- |---------- |--------------------------------  |-------- |
| size | 用于控制Loading组件内的字体、Icon尺寸大小 | string | medium / small / mini | — |
