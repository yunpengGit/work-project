...
:::demo 设置`size`属性可以渲染不同尺寸的复选框（仅对带有边框的复选框有效）。

```html
<template>
  <el-checkbox v-model="checked" border size="medium">中号</el-checkbox>
  <el-checkbox v-model="checked" border size="small">小号</el-checkbox>
  <el-checkbox v-model="checked" border size="mini">超小号</el-checkbox>
</template>
<script>
  export default {
    data() {
