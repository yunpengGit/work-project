...
:::demo 设置`size`属性可以渲染不同尺寸的单选框（仅对带有边框的单选框有效）。
```html
<template>
  <el-radio v-model="radio" label="1" border size="medium">中号</el-radio>
  <el-radio v-model="radio" label="2" border size="small">小号</el-radio>
  <el-radio v-model="radio" label="3" border size="mini">超小号</el-radio>
</template>
<script>
  export default {
    data () {
