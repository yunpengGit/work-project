    ...
    getOrganizationList () {
      this.$api('notice.organizationList').then(res => {
        ...
        ;(function iteration (arr) {
          that.organizationData = arr.map(e => {
            if (e.children.length > 0) {
              iteration(e.children)
            } else {
              e.isLeaf = true
            }
            return e
    ...
    async treeLazyLoad (node, resolve) {
      ...
      if (Array.isArray(data)) {
        ...
        if (Array.isArray(children) && children.length > 0) {
          ...
          // _data = children.map(e => {
          //   if (e.children.length === 0) e.isLeaf = true
          //   return e
          // })
          _data = children
          // console.log(children)
      ...
      this.$api('notice.searchOrganization', { ...params }).then(res => {
        this.organizationData = res.data.map(item => {
          return {
            id: item.id,
            label: item.name,
            isLeaf: (item.children && item.children.length > 0) ? true : false,
            // isLeaf: item.hasUsers,
