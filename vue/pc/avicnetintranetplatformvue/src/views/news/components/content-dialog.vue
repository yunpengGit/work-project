 ...
 * @LastEditors: peng
 * @LastEditTime: 2021-08-19 11:08:50
-->
<template>
  <div class="form-container">
    ...
    >
      ...
      <el-form-item label="正文：" prop="body">
        <div class="max1200">
          <tinymce-editor ref="editor" v-model="form.body" business-type="cms" :secret-level="Number(form.secretLevel)" :disabled="formDisabled" />
        </div>
      </el-form-item>
      <el-form-item label="附件：" prop="attachments">
        <div class="max1200">
          <enclosure ref="enclosure" :file-list="form.attachments" business-type="cms" :mj="form.secretLevel || '-1'" :disabled="formDisabled" @changeFileMj="changeFileMj" @upload="uploadFileSuccess" />
        </div>
      </el-form-item>
...
export default {
  components: { Treeselect, TinymceEditor, Enclosure, CoverUpload },
  props: {
    routerType: {
      type: String,
      default: () => {}
  ...
  computed: {
    ...
    secretLevel () {
      ...
      if (userSecretLevel === '1' || userSecretLevel === '') {
  ...
  created () {
    ...
    if (this.data) {
      this.$api('news.contentsQueryIds', { id: this.data.id }).then(res => {
    ...
    //   this.form.secretLevel = res.data
    // })
  },
  methods: {
    ...
    changeFileMj (row) {
      this.form.attachments = this.form.attachments.map(e => {
        if (e.id === row.id) {
    ...
    closedPage () {
      this.$store.commit('CLOSE_DIALOG', { uid: 'content-dialog' })
