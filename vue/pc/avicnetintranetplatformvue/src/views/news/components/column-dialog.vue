 ...
 * @LastEditors: caofeng
 * @LastEditTime: 2021-08-27 10:06:56
-->
<template>
...
export default {
  ...
  created () {
    ...
    if (this.data) {
      this.$api('news.catalogsQueryId', { id: this.data.id }).then(res => {
        const columnData = { ...res.data }
        this.isAdd = false
        columnData.catalogViewerPerms = columnData.catalogViewerPerms && columnData.catalogViewerPerms.map(ele => {
          ele.id = ele.viewerId
          ele.label = ele.viewerName
          return ele
        })
        columnData.catalogManagerPerms = columnData.catalogManagerPerms && columnData.catalogManagerPerms.map(ele => {
          ele.id = ele.managerId
          ele.label = ele.managerName
          return ele
        })
        this.form = columnData
      })
    }
  ...
  methods: {
      ...
      this.$refs.form.validate((isValidate) => {
        if (isValidate) {
          ...
          const arr = list && list.map(ele => {
            ele.viewerName = ele.label
            ele.viewerId = ele.id
            ele.viewerType = ele.viewerType ? ele.viewerType : (ele.isPerson ? 2 : 1)
            return ele
          })
          // console.log(this.form)
          this.form.catalogViewerPerms = arr
    ...
    closedPage () {
      this.$store.commit('CLOSE_DIALOG', { uid: 'column-dialog' })
