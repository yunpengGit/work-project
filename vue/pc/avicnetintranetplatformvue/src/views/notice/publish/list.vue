 ...
 * @LastEditors: zongmz
 * @LastEditTime: 2021-05-12 16:33:13
-->
<template>
  <div>
    <sp-table
...
export default {
  ...
  methods: {
    ...
    routeToNewTab (id) {
      window.open(`${process.env.VUE_APP_INDEX_LINK}/notice/detail?id=${id}&type=mySee`, '_blank')
    },
    formatTableCols () {
      this.tableCols = [
        {
          ...
          template: (scope) => {
            return <div on-click={() =>
              this.routeToNewTab(scope.row.id)
        ...
        },
        {
          label: 'MJ',
          template: (scope) => {
            return this.secretLevel.find(v => Number(v.dictValue) === scope.row.secretLevel).dictLabel
          }
        },
        {
          label: '已阅人数',
