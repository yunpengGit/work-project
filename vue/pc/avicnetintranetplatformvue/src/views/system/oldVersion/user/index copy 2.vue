  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-col :span="20" :xs="24">
        <sp-table :table-cols="cols" :api="api" :selection.sync="selection" keys="userName,phonenumber,userStatus,dateRangeValue">
          <div slot="top" class="opt-btns">
            ...
            <el-button v-hasPermi="['system:user:edit']" type="success" icon="el-icon-edit" size="mini" :disabled="selection.length != 1" @click="handleUpdate">修改</el-button>
            <el-button v-hasPermi="['system:user:remove']" type="danger" icon="el-icon-delete" size="mini" :disabled="!selection.length" @click="handleDelete">删除</el-button>
          ...
          <!-- <template>
            <sp-table :cols="colsC" :api="api" :selection.sync="selection" :layout="null"></sp-table>
          </template> -->
        </sp-table>
      </el-col>
    </el-row>
    <sp-dialog :title="title" width="600px" uid="user-dialog" />
    <!-- 用户导入对话框 -->
...
import Params, {
  parse
} from '@/components/utils/router-params'
...
export default {
  ...
  methods: {
    api (params) {
      return this.$api('users.listUser', params)
    },
    /** 查询部门下拉树结构 */
    getTreeselect () {
    ...
    handleNodeClick (data) {
      ...
      const bb = this.params.componentization(params)
      this.$router.push({
        query: {
          ...this.$router.query,
          ...bb
