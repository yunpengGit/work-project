    ...
    >
      <div>
        <el-form ref="form" label-position="left">
          <el-form-item v-if="editValue.sharerId" label="分享人：" label-width="120px">
            <span style="color: #191f24; font-weight: 500">{{
              editValue.sharerUserName
            }}</span>
          </el-form-item>
          <el-form-item v-else label="分享人：" label-width="120px">
  ...
  methods: {
    ...
    confirm () {
      if (!this.editValue.sharerId && !this.shareList.length) {
        return this.$message.error('请选择分享人')
      }
      if (
    ...
    confirmSelectedUserIds (data) {
      this.editValue[
        this.type === 'READWRITE' ? 'writePermsUserItems' : 'readPermsUserItems'
      ] = []
      data.forEach((ele) => {
        this.editValue[
          this.type === 'READWRITE'
