 ...
 * @LastEditors: peng
 * @LastEditTime: 2021-08-12 15:00:31
-->
<template>
  <div ref="componentBox" class="component-box">
    ...
    <ul v-if="list.length>0" class="component-box-cont notice-cont">
      <div v-if="config.dataField[0].link" :class="{'active':!!config.dataField[0].link}" @click="routeToNewTab(config.dataField[0].link)">
...
export default {
  ...
  mounted () {
    if (this.blockConfig && JSON.stringify(this.blockConfig) !== '{}' && this.blockConfig.componentAttr) {
      ...
      this.config.title = this.blockConfig.label
