<!--
 * @Author: aq.hu
 * @Date: 2021-05-11 11:42:36
 * @LastEditTime: 2021-07-01 13:55:36
 * @LastEditors: Please set LastEditors
 * @Description: 领导一周隐私设置
-->
<template>
    ...
    <el-dialog v-if="dialogVisible" :title="leaderId? '编辑':'新增'" :visible.sync="dialogVisible" class="person-dialog">
      ...
      <el-form ref="ruleForm" :model="form" :rules="rules">
        <el-form-item label="领导姓名：" prop="leader">
          <span v-if="leaderId" class="leader-name">{{ form.leader.length && form.leader[0].label }}</span>
          <div v-else class="selete-wrap">
            <el-tag v-for="(item,index) in form.leader" :key="index" closable type="info" @close="form.leader.splice(index,1)">{{ item.label }}</el-tag>
        ...
        <el-form-item label="可编辑者：" prop="editors">
          <div class="selete-wrap">
            <el-tag v-for="(item,index) in form.editors" :key="index" closable type="info" @close="form.editors.splice(index,1)">{{ item.label }}</el-tag>
  ...
  methods: {
    ...
    onEdit ({ leaderId }) {
      ...
      this.$api('leadersWeekly.authInfo', { leaderId }).then((res) => {
        ...
        this.form.leader = [{ id: leaderId, label: leaderName }]
        this.form.leaderType = leaderType
        this.form.showOrder = showOrder
        this.form.editors = editors.map(v => { return { id: v.editorId, label: v.editorName } })
      }).catch(() => {})
      this.dialogVisible = true
    },
    onSure () {
      this.$refs['ruleForm'].validate((valid) => {
        ...
        if (valid) {
          const req = {
            ...
            leaderName: this.form.leader[0].label,
            leaderType: this.form.leaderType,
            showOrder: this.form.showOrder,
            editors: this.form.editors.map(v => { return { editorId: v.id, editorName: v.label } })
          }
