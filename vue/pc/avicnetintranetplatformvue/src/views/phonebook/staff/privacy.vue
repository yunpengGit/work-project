        ...
        >
          <div slot-scope="{row}" class="checkbox-setting-wrap">
            <div v-for="item in row.privacyLevelRules" :key="item.id">
              ...
              <el-checkbox :value="isCheckAll(item)" @change="onCheckAll(row.viewerLevel, item)">全选</el-checkbox>
              <el-checkbox v-for="data in item.privacyFieldSettings" :key="item.id + data.fieldName" v-model="data.checked" @change="changeData(row.viewerLevel, item)">{{ data.fieldDesc }}</el-checkbox>
            </div>
          </div>
        </sp-table>
      </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    // 选择数据后将改变的数据塞入原始数据
    changeData (outLev, item) {
      const arr = [...this.list[this.activeName]]
      arr.map((t, i) => {
        if (t.viewerLevel === outLev) {
          t.privacyLevelRules.map((b, j) => {
            if (b.id === item.id) {
              this.list[this.activeName][i].privacyLevelRules[j].privacyFieldSettings = item.privacyFieldSettings
            }
          })
        }
      })
