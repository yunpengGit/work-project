<template>
  <div class="authorityManagement">
      ...
      <div class="authorityDiv">
        ...
        <div class="authorityLeft">
          <div v-show="rootNodeClickShow && !( modalSureBtnStatus=='add' || modalSureBtnStatus=='edit')">
            ...
            <a-button type="primary" @click="authorityAdd" icon="plus" :style="{marginLeft:'20px'}">添加子节点</a-button>
          </div>
          <div class="activetab2" v-if="activeTab == 2" v-show="!authorityAddModal && !rootNodeClickShow && defaultGroupShowState">
            ...
            <a-row :gutter="24" :style="{marginBottom:'46px'}">
              <a-col :push="6" :span="6">
                <a-button type="primary" @click="authorityAdd" icon="plus">添加子节点</a-button>
              </a-col>
              <a-col :push="5" :span="4">
                <a-button @click="authorityEdit" icon='edit'>编辑</a-button>
              </a-col>
              <a-col :push="5" :span="6">
                <a-button @click="authorityDele" icon='delete'>删除</a-button>
              </a-col>
            </a-row>
          </div>
          <div class="tabClassDiv" v-show="!addDivRoles && !authorityAddModal && !rootNodeClickShow && (defaultGroupShowState || activeTab!=2)">
            ...
            <a-button v-if="activeTab!=2" type="primary" icon="plus" @click="add">添加</a-button>
            <a-button v-if="activeTab==1" :style="{marginLeft:'10px'}" type="primary" @click="refreshAll" icon='sync'>刷新缓存</a-button>
            <a-button v-if="activeTab==3" :style="{marginLeft:'10px'}" type="danger" :disabled="batchDeleState" @click="batchDele" icon="delete">批量删除</a-button>
            <a-button v-if="activeTab==2" type="primary" @click="bindLink" icon='link'>绑定链接</a-button>
            <a-button v-if="activeTab==2" :style="{marginLeft:'10px'}" type="danger" :disabled="batchDeleState" @click="batchDele" icon="delete">批量删除</a-button>
            <a-table
              :columns="columns"
            ...
            >
              ...
              <span slot="selectTab1" slot-scope="text, record">
                <span class="btn">
                  <span class="blueSpan" @click="detailsRoles(record)"><a-icon type="profile"/>详情</span>
                  <span class="blueSpan" @click="editUriShow(record)"><a-icon type="edit"/>编辑</span>
                  <span class="redSpan" @click="deleRoles(record)"><a-icon type="delete"/>删除</span>
              ...
              <span slot="selectTab3" slot-scope="text, record">
                <span class="btn">
                  <span class="blueSpan" @click="detailsRoles(record)"><a-icon type="profile"/>详情</span>
                  <span class="blueSpan" @click="editUriShow(record)"><a-icon type="edit"/>编辑</span>
                  <span class="redSpan" @click="deleUri(record)"><a-icon type="delete"/>删除</span>
                </span>
              </span>
              <span slot="selectTab2" slot-scope="text, record">
                <span class="btn">
                  <span class="blueSpan" @click="detailsAuthority(record)"><a-icon type="profile"/>详情</span>
                  <span class="redSpan" @click="deleteByLimituriId(record)"><a-icon type="delete"/>移除</span>
                </span>
              </span>
            </a-table>
            <a-pagination
          ...
          <a-spin v-show="authorityAddModal" :spinning="loading">
            ...
            <a-form-model
              ...
              <div class="buttonFooter">
                <a-button type="primary" @click="authorityAddModalSave" icon='save'>保存</a-button>
    ...
    <a-spin v-show="addDivRoles" :spinning="loading">
      ...
      <a-form-model
        ...
        <a-row :gutter="16">
          <a-col :push="10" :span="12">
            <a-button type="primary" @click="addModalhandleOk" icon='save'>保存</a-button>
          </a-col>
          <a-col :span="6">
      ...
      <div v-show="modalSureBtnStatus=='detail'?true:false">
        ...
        <a-row :gutter="16">
          ...
          <a-col :span="6">
            <!-- <a-button @click="() => { addDivRoles === false }">取消</a-button> -->
            <a-button @click="deleRoles(tableRowIndex)" icon="delete">删除</a-button>
          </a-col>
        </a-row>
      </div>
    </a-spin>
    ...
    <a-modal v-model="authorityShowTable" title="绑定链接" @cancel="authorityStateCale" width='75%' :footer='null'>
      <div>
        ...
        <a-button type="primary" icon="link" @click="bindSelected()">绑定选中项</a-button>
        <a-table
          :columns="columns"
        ...
        >
          <span slot="selectTab2" slot-scope="text, record">
            <span class="btn">
              <span class="blueSpan" @click="detailsAuthority(record)"><a-icon type="profile"/>详情</span>
              <span class="blueSpan" v-if="!record.bindState" @click="bindSelected(record)"><a-icon type="link"/>绑定</span>
              <span style="color:#ccc;padding-right:6px" v-if="record.bindState"><a-icon type="link"/>已绑定</span>
            </span>
          </span>
        </a-table>
        <a-pagination
...
import Vue from 'vue'
import {
  saveUri, updateUri, deleteByIdUri, deleteByIdsUri, getPageByParamsUri, saveRoles, updateRoles, getPageByParamsRoles,
  deleteByIdRoles, refreshRoleCacheById, getLimitgroupListByRoleId, getLimitgroupTreeByName, getById,
  getLimituriListByLimitgroupId, saveGroup, updateGroup, getLimituriPageByLimitgroupId, saveBatch,
  deleteByIdGroup, deleteByLimitgroupIdAndLimituriIds
} from '@/api/service'
...
export default {
  ...
  methods: {
    getLimitgroupTree() {
      getLimitgroupTreeByName({ name: '' }).then(res => {
        if (res.code == '1000000') {
    ...
    querySearch(pageNumber, pageSize) {
      ...
      if (this.activeTab == 3) { // uri查询
        queryApi = getPageByParamsUri
      } else if (this.activeTab == 1) { // 角色查询
        queryApi = getPageByParamsRoles
      } else if (this.activeTab == 2) { // 已绑定的权限链接列表
        if (this.authorityShowTable) {
          queryApi = getPageByParamsUri
        } else {
          queryApi = getLimituriPageByLimitgroupId
    ...
    getLimitgroupList(params = {}) {
      // 不传参数查所有，传参数表示指定
      getLimitgroupListByRoleId(params).then(res => {
        if (res.code == '1000000') {
    ...
    deleUri(rowIndex) {
      Modal.confirm({
        title: '确认删除此权限链接？',
        onOk: () => {
          deleteByIdUri({ id: rowIndex.id }).then(res => {
            if (res.code == '1000000') {
              message.success(res.resultStr)
              this.querySearch()
            } else {
      ...
      Modal.confirm({
        ...
        onOk: () => {
          ...
          if (this.activeTab == 3) {
            ...
            api = deleteByLimitgroupIdAndLimituriIds
    ...
    refreshAll() {
      Modal.confirm({
        ...
        onOk: () => {
          ...
          refreshRoleCacheById({ ids: ids.join(',') }).then(res => {
    ...
    refreshRole(rowIndex) {
      Modal.confirm({
        title: '确认刷新指定角色对应的权限？',
        onOk: () => {
          refreshRoleCacheById({ id: rowIndex.id }).then(res => {
            if (res.code == '1000000' && res.model) {
              message.success(res.resultStr)
              this.querySearch()
            } else {
    ...
    deleRoles(rowIndex) {
      ...
      Modal.confirm({
        title: '确认删除此角色？',
        onOk: () => {
          deleteByIdRoles({ id: rowIndex.id }).then(res => {
            if (res.code == '1000000' && res.model) {
              message.success(res.resultStr)
              this.addDivRoles = false
              this.querySearch()
    ...
    authorityAdd() {
      ...
      this.modalSureBtnStatus = 'add'
      this.treePath = 'WEB运营端/系统管理'
      this.treePath = 'WEB运营端/系统管理'
      this.getParent(this.treeDataUserAuthority, this.tableRowIndex.id).map(item => { this.treePath += '/' + item.name })
      this.addParamAuthority.parentName = this.treePath
      // if (this.tableRowIndex.id == 0) {
      //   this.addParamAuthority.parentName = this.treePath
      // } else {
      //   this.addParamAuthority.parentName = this.treePath + (this.tableRowIndex.path || '')
      // }
    ...
    authorityEdit() {
      ...
      this.treePath = 'WEB运营端/系统管理'
      this.getParent(this.treeDataUserAuthority, this.tableRowIndex.id).map(item => { this.treePath += '/' + item.name })
      const num = this.treePath.lastIndexOf('/')
      this.addParamAuthority.parentName = this.treePath.substring(0, num) || ''
    ...
    authorityDele() {
      ...
      Modal.confirm({
        title: '确认删除此权限组？',
        onOk: () => {
          deleteByIdGroup({ id: this.tableRowIndex.id }).then(res => {
            if (res.code == '1000000' && res.model) {
              message.success(res.resultStr)
              this.defaultGroupShowState = false
              this.getLimitgroupTree()
    ...
    getAllLimituriListByLimitgroupId() {
      getLimituriListByLimitgroupId({ id: this.tableRowIndex.id }).then(res => {
        if (res.code == '1000000') {
    ...
    selectedTreeAuthority(selectedKeys) {
      ...
      if (selectedKeys[0] === 0) {
        this.tableRowIndex.name = 'WEB运营端/系统管理'
      ...
      getById({ id: selectedKeys[0] }).then(res => {
        if (res.code === 1000000) {
    ...
    deleteByLimituriId(rowIndex) {
      Modal.confirm({
        ...
        onOk: () => {
          ...
          deleteByLimitgroupIdAndLimituriIds(params).then(res => {
            if (res.code === 1000000) {
              message.success(res.resultStr)
              this.addModal = false
              this.querySearch()
