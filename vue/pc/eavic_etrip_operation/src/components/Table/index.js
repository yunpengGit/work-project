...
export default {
  ...
  methods: {
    ...
    loadData (pagination, filters, sorter) {
      ...
      const params = { pageNumber: parameter.pageNo, pageSize: parameter.pageSize }
      // console.log('parameter', parameter)
      const result = this.data(params)
      // 对接自己的通用数据接口需要修改下方代码中的 r.pageNumber, r.totalRow, r.list
      // eslint-disable-next-line
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          this.localPagination = (this.showPagination && Object.assign({}, this.localPagination, {
            current: r.pageNumber, // 返回结果中的当前分页数
            total: r.totalRow, // 返回结果中的总记录数
            showSizeChanger: this.showSizeChanger,
            pageSize: (pagination && pagination.pageSize) ||
          ...
          try {
            if ((['auto', true].includes(this.showPagination) && r.totalRow <= (r.pageNumber * this.localPagination.pageSize))) {
  ...
  render () {
    ...
    let showAlert
    if ((typeof this.alert === 'object' && this.alert !== null) && this.alert.show) {
