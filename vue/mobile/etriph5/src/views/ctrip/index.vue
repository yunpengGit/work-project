<template>
  <div :class="['ctrip-content', { hotel: isHotel }]">
    ...
    <!--对接携程，不要删除-->
    <dy-form ref="XCForm" :action="ctripData.url" />
    <!--对接携程，不要删除-->
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    toFlightOrder () { // 获取机票订单
      let type = this.isHotel ? 'HotelOrder' : 'FlightOrder'
      this.getCtripInfo(type)
    },
    toApprove () { // 获取订单审批
      this.getCtripInfo('AccreditOrder')
    },
    addOrder () {
      let type = this.isHotel ? 'HotelSearch' : 'FlightSearch'
      this.getCtripInfo(type)
    },
    // 获取携程信息接口
    getCtripInfo (xcFunc = 'FlightOrder') {
      this.$store.dispatch('addLoading')
      let orderType = this.$route.query.orderType ? this.$route.query.orderType : 2
      api.loginXc({
        ...
        this.$refs.XCForm.submit(res.commonModel.param)
      }).catch(e => {
        console.error(e)
        this.$store.dispatch('removeLoading')
      })
