...
[查看 demo](https://sunniejs.cn/vue-h5/#/) 建议手机端查看
...
git clone http://gitlab.avicnet.cn/goldenTeam/vue-h5.git
...
vue-h5 所有全局样式都在 `@/src/assets/css` 目录下设置
...
`vue.config.js` 配置使用 `css.loaderOptions` 选项,注入 `sass` 的 `mixin` `variables` 到全局，不需要手动引入
