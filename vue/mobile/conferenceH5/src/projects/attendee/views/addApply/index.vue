<!--
 * @Author: wanghz
 * @Date: 2021-08-14 16:13:18
 * @LastEditTime: 2021-09-10 01:32:48
 * @LastEditors: zhaohx
...
<template>
  <div class="add-attendee-container">
    <!-- 顶部tab -->
    <van-tabs v-model="tabIndex" :before-change="beforeChange" v-if="tabsData.length > 1">
...
color="#785900"
background="#FFF8E4"
          >*标信息请填写完整，确认无误后点击下一步。</van-notice-bar
        >
        <van-form>
          <van-cell-group>
            <div v-for="(ele, index) in basicData(1)" :key="index">
              <van-field
                ...
                :placeholder="`请输入报名人${ele.fieldLabel}`"
                input-align="right"
      ...
      <div v-show="tabIndex === 1 && param">
        <van-notice-bar mode="closeable" color="#785900" wrapable background="#FFF8E4">
          <div>· 为保障您的会务安排，信息请尽量填写完整</div>
          <div>· 如具体行程尚未确定，可在报名后通过"行程变更"修改</div>
        </van-notice-bar>
        <van-form>
          <van-cell-group>
            <div v-for="(ele, index) in basicData(2)" :key="index">
              ...
              <div v-if="ele.type === 9" class="pic-upload-list">
                ...
                <div class="pic-upload-list-btn">
                  <van-uploader v-model="fileList" :before-read="beforeReadPic(ele, 'single')">
                    <div class="pic-upload-list-style">+</div>
                  </van-uploader>
                  <img :src="fileList.length ? fileList[0].content : ''" @click="basicPicPreview(ele)" alt="" />
                </div>
      ...
      <div v-show="tabIndex === 2 && param">
        <van-notice-bar mode="closeable" color="#785900" wrapable background="#FFF8E4">
          <div>· 必选议程无法取消，自选议程可按需选择</div>
          <div>· 报名后如需变更参加议程，可在报名截止前通过“报名信息”修改</div>
        </van-notice-bar>
    ...
    <van-dialog
      v-model="noticeDialog.show"
      show-cancel-button
      :title="noticeDialog.title"
      confirmButtonText="我知道了"
      :showCancelButton="false"
      confirmButtonColor="rgba(30, 137, 255, 1)"
      width="290px"
    >
      <div class="van-dialog__message" style="height:200px;">
        <div style="text-align: left; ">{{ noticeDialog.msg }}</div>
      </div>
    </van-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    setKeep(toast = '') {
      ...
      // 头像处理
      console.log(this.param.signFormConfigFormatVO.photoUrl.fieldValue)
      if (
        ...
        console.log(this.param.signFormConfigFormatVO.photoUrl.fieldValue[0])
      }
      const data = this.param.signFormConfigFormatVO
      Object.keys(data).forEach(ele => {
    ...
    submit() {
      ...
      if (this.tabIndex === this.tabsData.length - 1) {
        ...
        this.$dialog
          .confirm({
            title: '是否提交报名',
            message: '如提交后需变更报名信息，可前往首页"报名信息"修改。',
                ...
                .then(resMsg => {
                  ...
                  if (!data) {
                    ...
                    if (externalVisibaleFlag === 0) {
                      ...
                      this.getForms2().then(resolve => {
                        if (resolve) {
                          this.findEle('attendeeName').fieldValue = attendeeName
                          this.findEle('phone').fieldValue = phone
                          this.findEle('phoneForMsg').fieldValue = phone
                        }
                      })
                    }
                    // else {
                      ...
                      if (externalVisibaleFlag === 0) {
                        ...
                        this.getForms2().then(resolve => {
                          if (resolve) {
                            this.findEle('attendeeName').fieldValue = attendeeName
                            this.findEle('phone').fieldValue = phone
                            // 当没有填写[常用手机号]或者表单配置[常用手机号]不显示时，默认传[手机号]字段值
                            this.findEle('phoneForMsg').fieldValue = phone
                          }
                        })
                      }
      ...
      return new Promise(resolve => {
        ...
        this.$api({
          ...
          if (res.code === 2000) {
            ...
            this.param = this.paramHandler(data)
            // 本人报名-基本信息回填
            if (Number(this.routeParams.replaceIdent) === 2) {
              this.$api({
                ...
                if (res.code === 2000) {
                  const data = res.result
                  if (data.ssoUserName) {
                    this.findEle('attendeeName').fieldValue = data.ssoUserName
                  }
                  this.findEle('phone').fieldValue = data.mobile
                  this.findEle('sex').fieldValue = data.sex
                  this.findEle('organizationName').fieldValue = data.organizationName
                  this.findEle('position').fieldValue = data.position
                  // 本人报名-获取草稿
                  if (Number(this.routeParams.replaceIdent) === 2) {
                    this.userInfoBlurHandler()
                  }
    ...
    getRegSettings() {
      ...
      this.$api({
        ...
        if (res.code === 2000) {
          const data = res.result
          this.regSettingsData = data
          if (this.regSettingsData.notice) {
            this.noticeDialog.show = true
            this.noticeDialog.msg = this.regSettingsData.notice
          }
        }
      })
    },
