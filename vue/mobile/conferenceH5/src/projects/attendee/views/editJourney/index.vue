 ...
 * @LastEditTime: 2021-09-10 01:34:02
 * @LastEditors: zhaohx
 * @Description: In User Settings Edit
-->
<template>
  <div class="add-attendee-container">
    <div class="main-container" v-if="param">
      <!-- 行程信息 -->
      <div>
        <!-- 申请人 -->
        <div style="height: 76px; background: #fff; padding: 12px 0">
          <div style="padding-left: 16px; font-size: 14px; color: rgba(25, 31, 36, 0.64); height: 20px">申请人</div>
          <div style="padding-left: 16px; font-size: 18px; height: 36px; line-height: 36px">
            {{ routeParams.attendeeName }}
          </div>
        </div>
        <van-form>
          <van-cell-group>
        ...
        <div class="attendant-result-container" v-if="attendantResult.length">
          ...
          >
            <div class="name">
              ...
              <span>{{ ele.fixAttributeCommonVO.organizationName }}</span>
            </div>
...
export default {
                  ...
                  if (eleCustom.type === 8 && eleCustom.showFlag) {
                    ...
                    if (eleCustom.fieldValue) {
                      ...
                      if (selectData && selectData.length) {
                        eleCustom.fieldValueStr = selectData.find(
                          eleFind => Number(eleFind.optionValue) === Number(eleCustom.fieldValue)
                        ).optionText
                      }
                    }
                    result.push(eleCustom)
  ...
  methods: {
    ...
    verifyTime() {
      ...
      console.log(startDate, endDate, 1111)
        ...
        this.$api({
          ...
          if (res.code === 2000) {
            ...
            if (Number(data.signStatus) === 1 || Number(this.attendeeOrRecordData.signStatus) === 1) {
              ...
              // this.paramsRequire.updateFlag = 4
            }
            // this.param = data
            resolve(true)
