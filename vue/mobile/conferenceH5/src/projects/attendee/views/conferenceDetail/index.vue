 ...
 * @LastEditTime: 2021-07-06 10:00:37
-->
<template>
  <div class="detail-container" v-if="conferenceData">
    <div :style="{ 'padding-bottom': showApplyBtn || routeParams.isWeb ? '70px' : '0' }">
      <div class="pic">
      ...
      <div class="contacts detail-item" v-if="conferenceData.contactsVOMS.length">
        <h3 class="detail-title-pic">
    ...
    <div class="bottom" v-if="!routeParams.isWeb">
      <van-button
        block
        type="info"
        :disabled="applyBtnText != '我要报名'"
        @click="showBtnPopup = true"
        v-if="showApplyBtn"
        >{{ applyBtnText }}</van-button
      >
    ...
    <div class="bottom" v-if="routeParams.isWeb">
      <van-button block type="default" @click="linkPage(0)" style="margin-right: 16px">进入会议</van-button>
      <van-button block type="info" @click="linkPage(1)">我要报名</van-button>

      <van-popup v-model="showBtnPopup" position="bottom" round>
        <div class="popup-container">
          <ul class="filter-condition">
            <li class="filter-item" v-for="ele in btnData" :key="ele.type" @click="linkWebApply(ele.index)">
              {{ ele.text }}
            </li>
          </ul>
          <div class="filter-submit-btn">
    ...
    if (this.routeParams.isWeb) {
      ...
      loadJS(webSsourl, () => {
        if (conferenceResult !== null && conferenceResult !== undefined) {
          this.conferenceData = conferenceResult
          this.$api({
            url: 'attendeeApi.getDownloadUrl',
            data: {
              filePath: conferenceResult.coverVOs[0].coverUrl
            }
          }).then(res => {
            if (res.success) {
              this.conferenceData.coverVOs[0].downloadUrl = res.result
            }
          })
        }
      })
    ...
    linkPage(type) {
      ...
      if (token) {
        ...
        this.$router.push({
          ...
          query: {
            [this.$enc('conferenceId')]: this.$enc(conferenceId),
            [this.$enc('conferenceName')]: this.$enc(this.conferenceData.name)
          }
        })
      }
    },
