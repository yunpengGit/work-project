 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-31 17:42:42
-->
<template>
  <div class="add-driver-container">
    ...
    <van-dialog v-model="customDinnerShow" show-cancel-button :beforeClose="beforeClose">
      <div class="custom-info-container">
        <van-field required label="联系人" v-model="params.contactPerson" placeholder="请输入联系人" class="customInput"/>
        <van-field required label="联系电话" v-model="params.contactPhone" placeholder="请输入联系电话" class="customInput"/>
      </div>
    </van-dialog>
    <van-popup
      v-model="submitShow"
...
export default {
  ...
  created() {
    ...
    this.params.attendeeTaskId = taskId
    // this.params.conferenceId = conferenceId
    // const id = this.routeParams.id
    const type = this.routeParams.taskType
    const data = JSON.parse(sessionStorage.getItem('carInfo'))
    if (type === '61') {
      this.paramsVal.typeVal = '接站用车'
      this.params.type = 1
    } else if (type === '62') {
      this.paramsVal.typeVal = '送站用车'
      this.params.type = 2
    } else if (type === '63') {
      this.paramsVal.typeVal = '会议用车'
      this.params.type = 3
    }
    let startName = ''
    let endName = ''
    ...
    this.paramsVal.carCard = data.platNum
    this.paramsVal.carName = data.vehicleTypeVO.name
    this.paramsVal.seatNum = data.vehicleTypeVO.seatNum
    this.paramsVal.color = data.vehicleColorStr
    this.paramsVal.driverName = data.driverVO ? data.driverVO.name ? data.driverVO.name : '' : ''
    this.paramsVal.driverPhone = data.driverVO ? data.driverVO.phone ? data.driverVO.phone : '' : ''
    this.params.conferenceId = conferenceId
    this.params.attendees = [{ id: attendeeId }]
    this.params.attendeeId = attendeeId
    this.params.driverId = data.driverVO ? data.driverVO.id ? data.driverVO.id : '' : ''
    this.params.vehicleId = data.vehicleTypeVO.id ? data.vehicleTypeVO.id : ''
    // this.$api({
  ...
  methods: {
    ...
    beforeClose(action, done) {
      if (action === 'confirm') {
        const contactPerson = this.params.contactPerson
        const contactPhone = this.params.contactPhone
    ...
    startChoice(val) {
      ...
      if (!this.params.endTime) {
        this.paramsVal.carCardTips = '请先选择送达时间'
      } else {
        if (this.params.startTime) {
