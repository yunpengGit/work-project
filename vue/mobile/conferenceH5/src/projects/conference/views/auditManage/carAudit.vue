 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-31 09:17:21
-->
<template>
  <div class="car-audit">
        ...
        <div v-for="(ele, index) in data.data" :key="index" class="msg-item" @click="linkTo(ele, index)">
          <div class="audit-item-title">
            <div class="title-text">
              ...
              <span
                >{{
                  ele.data.vehicleTaskType === '1'
                    ? '接站用车'
                    : ele.data.vehicleTaskType === '2'
                    ? '送站用车'
                    : ele.data.vehicleTaskType === '3'
                    ? '会议用车'
                    : '专车'
                }}
              </span>
          ...
          <div class="audit-item-content">
            <div class="content-title">
              {{ ele.data.vehicleTypeName }} | {{ ele.data.vehicleColor }}&nbsp;&nbsp;{{ ele.data.platNum }}
            </div>
            <div class="time">
              <span class="label">接站时间</span>
              <span class="value">{{ filterTime(ele.data.startTime) }}</span>
            </div>
            <div class="place">
              <span class="label">接站地点</span>
              <span class="value">{{ ele.data.startAddress }}</span>
            </div>
            <div class="person" v-if="ele.data.attendees.length">
              <span class="label">乘车人</span><span class="value">{{ filterAttendees(ele.data.attendees) }}</span>
            </div>
            <div class="place">
...
:disabled="checkedCount === 0 && !batchShowBtnFlag"
    ...
    <van-overlay :show="batchChoiceShow" z-index="9" @click="batchChoiceShow = false">
      <div class="batch-choice" @click.stop>
        <dl>
          <dt>正在审核用车安排</dt>
          <dd>请选择审核意见，用车安排会在审核通过后自动发布，乘车人可通过“会议安排”实时查看</dd>
        </dl>
        <ul>
...
export default {
    ...
    return {
      ...
      params: {
        ...
        httpParams: {
          ...
          type: 1, // 车房餐证任务类型 1用车 2住宿 3用餐 4票证
          total: 0,
          pageNum: 1,
          pageSize: 10
        }
  ...
  methods: {
    ...
    filterTime(time) {
      ...
      const todayOrtomo = this.todayOrtomo(newStr.slice(0, 10))
      if (todayOrtomo) {
        newStr = newStr.slice(0, 10) + ' ' + '(' + todayOrtomo + ')' + ' ' + newStr.slice(10, newStr.length)
      }
      return newStr
    },
    // 判断是否为今天和明天
