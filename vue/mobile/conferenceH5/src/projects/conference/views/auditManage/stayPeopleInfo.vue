 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-31 09:19:41
-->
<template>
  <div class="message-detail-container">
    <div class="car-people-search">
      ...
      <div class="car-people-top">
        <span class="car-people-model">{{ auditItem.data.modelName }}</span>
        <span class="point" v-if="auditItem.data.roomNo"> · </span>
        <span class="car-people-room">{{ auditItem.data.roomNo }}</span>
        <span class="line" v-if="auditItem.data.hotelName">|</span>
        <span class="car-people-hotel"> {{ auditItem.data.hotelName }}</span>
      </div>
    </div>
    <list
      ref="list"
      ...
      <template v-slot:item="data">
        <div v-for="(ele, index) in data.data" :key="index" class="msg-item">
          <div class="photo">
            <img
              :src="
                ele.attendee.fixAttributeCommonVO.photoUrl
                  ? ele.attendee.fixAttributeCommonVO.photoUrl
                  : ele.attendee.fixAttributeCommonVO.sex === '2'
                  ? femaleSex
                  : male
              "
              alt=""
          ...
          <div class="car-people-info">
            <div
              ...
              :style="{ 'margin-bottom': ele.attendee.tagVOs && ele.attendee.tagVOs.length ? '7px' : '16px' }"
            >
              <span class="name">{{ ele.attendee.fixAttributeCommonVO.attendeeName }}</span
              ><span class="line" v-if="ele.attendee.fixAttributeCommonVO.position"></span
              ><span class="position">{{ ele.attendee.fixAttributeCommonVO.position }}</span>
              <span class="msg-tip"></span>
            </div>
            <div class="tag" v-if="ele.attendee.tagVOs && ele.attendee.tagVOs.length">
              <span
                class="tag-item"
                v-for="(eleChild, indexChild) in ele.attendee.tagVOs"
                :key="indexChild"
            ...
            <div class="company">{{ ele.attendee.fixAttributeCommonVO.organizationName }}</div>
            <div class="phone" v-if="ele.attendee.fixAttributeCommonVO.phone">
              <span>{{
                ele.phoneShow ? ele.attendee.fixAttributeCommonVO.phone : phone(ele.attendee.fixAttributeCommonVO.phone)
              }}</span>
...
export default {
  ...
  data() {
    return {
      ...
      auditItem: {},
      params: {
        httpUrl: 'message.getRoomAttendees',
        type: 'up',
        httpParams: {
          conferenceId: '',
          roomId: '',
  ...
  created() {
    ...
    this.$nextTick(() => {
      ...
      this.params.httpParams.roomId = this.auditItem.data.roomId
      this.params.httpParams.startDate = this.auditItem.data.startDate.slice(0, 10)
      this.params.httpParams.endDate = this.auditItem.data.endDate.slice(0, 10)
    })
  },
  methods: {
    // 查询
