 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-31 09:17:35
-->
<template>
  <div class="car-audit">
        ...
        <div v-for="(ele, index) in data.data" :key="index" class="msg-item" @click="linkTo(ele, index)">
          <div class="audit-item-title">
            <div class="title-text">
              ...
              <span>{{ ele.data.typeCode }}安排</span>
            </div>
          ...
          <div class="audit-item-content">
            <div class="content-title">{{ ele.data.dinnerRoom }}</div>
            <div class="time">
              <span class="label">用餐日期</span>
              <span
class="value"
                >{{ formatCode(ele.data.dinnerStartDate) }}
                <span v-if="todayOrtomo(ele.data.dinnerStartDate)">({{ todayOrtomo(ele.data.dinnerStartDate) }})</span>
                - {{ formatCode(ele.data.dinnerEndDate)
                }}<span
v-if="todayOrtomo(ele.data.dinnerEndDate)"
                  >({{ todayOrtomo(ele.data.dinnerEndDate) }})</span
                ></span
              >
            </div>
            <div class="time">
              <span class="label">用餐时间</span>
              <span class="value">{{ ele.data.startTime.slice(0, 5) }} - {{ ele.data.endTime.slice(0, 5) }}</span>
            </div>
            <div class="place">
              <span class="label">餐位数</span>
              <span class="value">{{ ele.data.seatNum }}</span>
            </div>
            <div class="person" v-if="ele.data.attendees.length">
              <span class="label">用餐人员</span><span class="value">{{ filterAttendees(ele.data.attendees) }}</span>
            </div>
            <div class="place">
    ...
    return {
      ...
      params: {
        ...
        httpParams: {
          ...
          type: 3, // 车房餐证任务类型 1用车 2住宿 3用餐 4票证
          total: 0,
          pageNum: 1,
          pageSize: 10
        }
