 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-11 12:27:22
-->
<template>
  ...
  methods: {
    beforeClose(action, done) {
      if (action === 'confirm') {
        ...
        const attendees = []
        attendeeIdList.forEach(ele => {
          attendees.push({ id: ele })
        })
        const params = {
          contactPerson: '',
          contactPhone: ''
        ...
        params.taskId = this.routeParams.taskId
        params.attendees = attendees
        this.$dialog.confirm({
          title: `将添加${attendeeIdList.length}名参会人员，使用相同的联系人`,
          message: '确认添加？',
    ...
    onSelect(item) {
      ...
      if (id === 0) {
        this.$dialog.confirm({
          title: `将添加${attendeeIdList.length}名参会人员，均与本人联系`,
          message: '确认添加？',
          className: 'customDialog'
        }).then(() => {
          // 新增 直接联系用车人
...
.manage-stay-staff-container::v-deep {
  min-height: 100%;
