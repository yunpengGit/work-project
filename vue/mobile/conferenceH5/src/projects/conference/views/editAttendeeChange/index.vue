 ...
 * @LastEditTime: 2021-09-10 04:58:02
 * @LastEditors: zhaohx
    ...
    <div :class="tabsData.length>1?'main-container':''" v-if="param">
      <div class="add-attendee" v-show="tabIndex === 0 && param">
          ...
          <van-cell-group>
            ...
            >
              <van-field
                ...
                v-if="ele.type === 1 && ele.fieldCode !== 'photo' && ele.fieldCode !== 'phoneForMsg' && ele.fieldCode==='cardNum'"
              ...
              <van-field
                :readonly="ele.fieldCode === 'attendeeName' || ele.fieldCode === 'phone'"
                v-if="ele.type === 1 && ele.fieldCode === 'photo'"
                v-model="ele.fieldValue"
                :required="ele.requireFlag === 1 ? true : false"
                :label="ele.fieldLabel"
                :placeholder="`请输入${ele.fieldLabel}`"
        ...
        <div class="tags-container">
          ...
          <div class="tags">
            <ul class="tags-list">
              <li v-for="(ele, index) in param.tagVOs" :key="index" @click="labelDelete(index)">
                {{ ele.name }}
                <van-icon name="cross" />
              </li>
            </ul>
...
export default {
  ...
  created() {
    ...
    const getData = async function() {
      ...
      this.$api({
        ...
        this.isLoading = true
        if (!this.param.agendaIds) {
          this.param.agendaIds = []
        }
        // 获取议程
        this.getAddAgenda()
  ...
  methods: {
    ...
    choiceAttendant(index) {
      ...
      console.log(this.findEle('honourId'))
      // linkAttendeeList
        ...
        data.forEach(ele => {
          if (ele.id) {
            ...
            if (!ele.freeFlag) {
              ...
              // this.param.agendaIds.push(ele.id)
              if (ele.groupId) {
                setTimeout(() => {
            ...
            if (this.param.agendaVOs.length && agendaChecked) {
    ...
    choiceAgenda(ele, eleChild) {
      if (eleChild.freeFlag) {
        ...
        console.log(this.param.agendaIds)
      }
    },
    agendaShow(index) {
    ...
    setKeep(toast = '') {
      ...
      const startDate = this.findEle('arriveTimeCustom').fieldValue
      const endDate = this.findEle('returnTimeCustom').fieldValue
      const startTime = this.arriveTime
      const endTime = this.returnTime
      if (startDate) {
      ...
      console.log(this.paramsRequire.submitFlag)
      this.$api({
        ...
        if (res.code === 2000 && res.success) {
          if (toast) {
            this.$toast(toast)
            if (this.paramsRequire.submitFlag === 1 || toast === '修改参会人成功') {
              this.$toast(toast)
              // 添加成功后 刷新当前页面
              setTimeout(() => {
                this.$router.go(-1)
    ...
    submit() {
      ...
      //     className: 'customDialog'
      //   }).then(() => {
      //     console.log(this.param.signStatus)
    ...
    // 标签删除
    labelDelete(index) {
      const id = this.param.tagVOs[index].id
      this.param.tagVOs.splice(index, 1)
      const choiceIndex = this.param.tagIds.findIndex(ele => ele === id)
      this.param.tagIds.splice(choiceIndex, 1)
    },
    // 自定义单选
