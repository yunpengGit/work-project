 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-12 21:52:26
-->
<template>
  <div class="add-driver-container">
    ...
    <van-action-sheet
      ...
      @select="onSelect"
    />
    <div class="single-btn-bottom single-btn-bottom-allow" v-if="this.routeParams.addUserFlag">
      <van-button
        type="info"
        @click="submit"
      >
    ...
    <div class="single-btn-bottom" v-else>
      <van-button
        @click="deleteCar"
      >
        删除车辆
...
export default {
  ...
  computed: {
    ...
    platNumUpper: {
      get() {
        return this.params.platNum
      },
      set(val) {
        this.params.platNum = val.toUpperCase()
      }
    }
  },
  created() {
    ...
    this.$api({
      ...
      if (res.code === 2000) {
        ...
        this.paramsVal.vehicleTypeStr = data.vehicleTypeVO.name
        this.paramsVal.seatNumStr = data.vehicleTypeVO.seatNum
        this.paramsVal.vehicleColorStr = data.vehicleColorStr
        ...
        this.paramsVal.companyStr = data.driverVO ? data.driverVO.vehicleCompanyVO.name ? data.driverVO.vehicleCompanyVO.name : '' : ''
      }
    })
  },
  methods: {
