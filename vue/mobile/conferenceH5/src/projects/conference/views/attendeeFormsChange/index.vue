 ...
 * @LastEditTime: 2021-09-10 01:23:42
 * @LastEditors: zhaohx
...
<template>
  <div class="add-attendee-container">
    <div class="main-container" v-if="isLoading">
      <div class="examine-status" v-if="param.changeStatus !== -1 && examineStatusShow">
        <dl>
          <dt>
            变更详情：
            <van-icon name="cross" @click="examineStatusShow = false"/>
          </dt>
          <dd v-html="locusContent(param.changeContent)"></dd>
        </dl>
      </div>
      <div class="attendee-item">
        ...
        <!-- <div class="tags-container">
          ...
          <div class="tags">
            <ul class="tags-list">
              <li v-for="(ele, index) in labelChoiceData" :key="index">
                {{ ele.name }}
                <van-icon name="cross" />
              </li>
            </ul>
              ...
              >
                <div class="content" v-if="eleChild._checked">
                  <div class="time">
                    ...
                    <div class="choice">
                      <!-- <img :src="eleChild.freeFlag ? eleChild._checked ? choose : chooseDefault : chooseForbid" alt=""> -->
                    </div>
                  </div>
                  <h4>{{ eleChild.name }}</h4>
                    ...
                    >
                      <div v-if="eleBrat._checked">
                        <div class="time">
                          ...
                          <div class="choice">
                            <!-- <img :src="eleBrat.freeFlag ? eleBrat._checked ? choose : chooseDefault : chooseForbid" alt=""> -->
                          </div>
                        </div>
                        <h4>{{ eleBrat.name }}</h4>
    ...
    <!-- 报名待审核 -->
    <!-- <div class="single-btn-bottom" v-if="isLoading && param.signStatus === 1">
      <van-button
        @click="$router.push({
          name: 'editAttendee',
          query: {
    ...
    <!-- 审核通过 -->
    <!-- <div class="single-btn-bottom" v-if="isLoading && param.signStatus === 3 && param.changeStatus === -1">
      <van-button
        @click="$router.push({
          name: 'editAttendee',
          query: {
    ...
    <!-- 变更待审核 -->
    <!-- && (param.changeStatus === -1 || param.changeStatus === 2)" -->
    <div class="single-btn-bottom" v-if="isLoading && param.changeStatus === 1" :class="routeParams.isBacklog ? '' : 'single-btn-bottom-allow'">
      <van-button
        :type="routeParams.isBacklog ? 'default' :'info'"
        @click="$router.push({
      ...
      >修改</van-button>
      <van-button
        v-if="routeParams.isBacklog"
        type="info"
        @click="examineHandler"
      >审核</van-button>
    </div>
    <!-- 禁止报名/报名驳回 -->
    <!-- <div class="single-btn-bottom single-btn-bottom-allow" v-if="isLoading && param.signStatus === -3">
      <van-button
        type="info"
        @click="allowSignUp"
      >恢复报名资格</van-button>
...
export default {
  ...
  data() {
    return {
      web, video, voice, ppt, txt, folder, picture, excel, word, zip, unknow, pdf, chooseForbid, choose, chooseDefault, checked,
              ...
              if (data[ele] && data[ele].groupId === index && data[ele].showFlag === 1) {
                ...
                if (ele === 'sex') {
                  if (data[ele].fieldValue) {
                    console.log(data[ele].fieldValue, this.sexData.find(eleMultiple => String(eleMultiple.valCode) === String(data[ele].fieldValue)).valName)
  ...
  created() {
    console.log(111, this.routeParams)
  ...
  methods: {
          ...
          if (res.code === 2000) {
            this.$api({
              ...
              data: {
                ...
                attendeeId: this.param.verifyUserId,
    ...
    onSelectExamine(item) {
      ...
      // const signStatus = 3 // 这里没有 signStatus了
      if (type === '通过') {
        this.approved()
      } else if (type === '不通过') {
        this.examineShow = true
        const conferenceId = this.routeParams.id
        const attendeeId = this.param.verifyUserId
        this.$api({
    ...
    timeComfire(val) {
      ...
      if (key === 'arriveTime') {
        ...
        this.findEle(key).fieldValue = this.findEle('arriveTimeCustom').fieldValue ? `${this.findEle('arriveTimeCustom').fieldValue.slice(0, 10)} ${this.formatHourSymbol(hour, minutes)}` : this.formatHourSymbol(hour, minutes)
      } else if (key === 'returnTime') {
        const hour = val[0]
        const minutes = val[1]
        this.findEle(key).fieldValueStr = `${val[0]}${val[1]}`
        this.findEle(key).fieldValue = this.findEle('returnTimeCustom').fieldValue ? `${this.findEle('returnTimeCustom').fieldValue.slice(0, 10)} ${this.formatHourSymbol(hour, minutes)}` : this.formatHourSymbol(hour, minutes)
        ...
        this.findEle(key).fieldValue = this.findEle('arriveTime').fieldValue ? `${this.formatCodeSymbol(val[0])} ${this.findEle('arriveTime').fieldValue.slice(-8)}` : `${this.formatCodeSymbol(val[0])} 00:00:00`
        ...
        this.findEle(key).fieldValue = this.findEle('returnTime').fieldValue ? `${this.formatCodeSymbol(val[0])} ${this.findEle('returnTime').fieldValue.slice(-8)}` : `${this.formatCodeSymbol(val[0])} 00:00:00`
      }
      setTimeout(() => {
