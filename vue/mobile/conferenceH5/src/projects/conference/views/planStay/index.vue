 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-09 19:09:39
-->
<template>
  <div class="plan-container">
    <ul class="plan-list" v-if="getData">
      ...
      >
        <div class="location">
          <h3>
            <span class="time">{{ ele.roomFormVO.modelName }}</span>
            <i />
            {{ ele.roomNo }}
          </h3>
          <div class="detail">
            酒店位置：<span>{{ ele.roomFormVO.hotelVO.address ? ele.roomFormVO.hotelVO.address : '-' }}</span>
          </div>
          <div class="detail">
            酒店名称：<span>{{ ele.roomFormVO.hotelVO.name ? ele.roomFormVO.hotelVO.name : '-' }}</span>
          </div>
        </div>
        <div class="car">
          <div class="detail flex">
            入住时间：{{ ele.startDate }} | {{ ele.endDate }}
            <span>共 {{ differDate(ele.startDate, ele.endDate) }} 天</span>
          </div>
        </div>
      </li>
    </ul>
  ...
  computed: {
    differDate() {
      return function(startTime, endTime) {
        ...
        return Math.ceil((eTime - sTime) / oneDay)
      }
    }
  },
  created() {
