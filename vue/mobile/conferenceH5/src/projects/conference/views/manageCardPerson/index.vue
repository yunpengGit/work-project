 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-11 12:27:55
-->
<template>
  <div class="manage-stay-staff-container">
          ...
          >
            <div class="left-item">
              <van-checkbox
                ...
                :ref="'checkbox'+ i"
                class="list-check"
                :name="ele.attendee.id"
              />
              <dl>
                <dt>
                  {{ ele.attendee.fixAttributeVO.attendeeName }}
                  <div
                    class="status"
                ...
                <dd>{{ ele.attendee.fixAttributeVO.organizationName }}</dd>
                <dd><span>职务：</span>{{ ele.attendee.fixAttributeVO.position ? ele.attendee.fixAttributeVO.position : '-' }}</dd>
                <dd><span>电话：</span><span class="contact">{{ ele.attendee.fixAttributeVO.phone }}</span></dd>
              </dl>
            </div>
            <van-popover
              v-if="!changeStatus"
    ...
    <div class="single-btn-bottom" v-if="!changeStatus">
      ...
      <van-button
        ...
        @click="$router.push({
          ...
          query: {
            ...
            [$enc('cardStatus')]: $enc(routeParams.cardStatus),
            [$enc('cardId')]: $enc(routeParams.cardId)
          }
        })"
      >添加用证人员</van-button>
    </div>
    <div class="status-bottom" v-if="changeStatus">
      ...
      <div>
        <van-button @click="changeStatusCancel">取消</van-button>
        <van-button
          type="info"
          :disabled="changePeople.length === 0"
          @click="statusShow = true"
      >
        确定
        <span v-if="changePeople.length !== 0">({{changePeople.length}})</span>
      </van-button>
      </div>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      jobData: [],
      params: {
        httpUrl: 'manage.getCardPersonList',
        type: 'up',
        // 模拟数据
        httpParams: {
          // 证件分配状态
  ...
  methods: {
    ...
    checkGroupChange() {
      const selectLen = this.changePeople.length
    ...
    cardCancel(data) {
      this.$dialog.confirm({
        ...
        const obj = {
          cardStatus: 6,
          cardAttendeeList: [{
            attendee: { id: data.attendee.id },
            card: { id: this.routeParams.cardId }
          }]
        }
        this.$api({
          url: 'manage.changeCardStatus',
          data: obj
        }).then(res => {
          if (res.code === 2000) {
            this.$api({
              url: 'manage.getCardPersonList',
    ...
    // 修改状态确定
    statusComfirm() {
      this.statusParams.cardStatus = this.selectStatus
      this.statusParams.cardAttendeeList = []
      this.changePeople.map(t => {
        const obj = {
          card: { id: this.routeParams.cardId },
          attendee: { id: '' }
        }
        obj.attendee.id = t
        this.statusParams.cardAttendeeList.push(obj)
      })
      this.$dialog.confirm({
        title: `将修改${this.changePeople.length}名人员的证件分配状态`,
        message: '确认修改',
        className: 'customDialog'
      }).then(() => {
        this.$api({
