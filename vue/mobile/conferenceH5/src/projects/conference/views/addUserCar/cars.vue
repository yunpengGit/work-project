 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-12 21:39:59
-->
<template>
      ...
      <template v-slot:item="data">
        ...
        >
          ...
          <div class="content">
            ...
            <dd>
              <span>司机：</span>
              <span>{{ ele.driverVO ? ele.driverVO.name : '-' }}</span>
              <i />
              <span class="contact">{{ ele.driverVO ? ele.driverVO.phone : '-' }}</span>
            </dd>
          </dl>
          <div class="cars-company">车辆所属公司：{{ ele.vehicleCompanyVO ? ele.vehicleCompanyVO.name : '-' }}</div>
          </div>
        </div>
      </template>
    </list>
...
export default {
  ...
  created() {
    ...
    this.$nextTick(() => {
      ...
      this.params.httpParams.startTime = this.startTime
      this.params.httpParams.endTime = this.endTime
    })
    this.getColor()
    this.getSeat()
    // this.$api({
