 ...
 * @LastEditors: zhaohx
 * @LastEditTime: 2021-08-26 16:39:15
-->
<template>
  <div class="add-driver-container">
    <van-form>
      ...
      <van-cell-group>
        <van-field
          ...
          placeholder="请选择任务类型"
          input-align="right"
          :right-icon="routeParams.hide ? '' : 'arrow'"
          @click="typeChoice"
        />
         <!-- <van-field
          readonly
    ...
    <div class="cars-choice-container" v-if="carsChoiceFlag || driversChoiceFlag">
      ...
      <drivers
        ...
        :routeParams="routeParams"
        :startTime="params.getOnVehicleTime"
        :endTime="params.endTime"
...
export default {
  ...
  created() {
    ...
    if (taskType) {
      this.paramsVal.typeVal = taskType === '61' ? '接站送车' : taskType === '62' ? '送站送车' : taskType === '63' ? '会议送车' : ''
      this.params.type = taskType === '61' ? 1 : taskType === '62' ? 2 : taskType === '63' ? 3 : ''
    }
    let startName = ''
    let endName = ''
  ...
  methods: {
    ...
    typeChoice() {
      this.routeParams.hide ? '' : this.actionShow = true
    },
    closePopup() {
    ...
    // 出发时间-上车时间-送达时间 验证 规则：上车时间 < 送达时间
    dateCheck() {
    ...
    arriveChoice(val) {
      ...
      this.dateCheck()
      if (!this.params.startTime) {
        this.paramsVal.carCardTips = '请选择接站时间'
      } else {
        if (this.params.endTime) {
          this.paramsVal.carCardTips = '请选择车牌号'
        }
