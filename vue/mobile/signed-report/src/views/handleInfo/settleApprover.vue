<template>
    <div>
      ...
      <div class="content">
        ...
        <Panel>
          <RadioList :list="list" listType="groupTransferred"/>
        </Panel>
      </div>
      <ChosenBox :list="list"  listType="groupTransferred" :sourceData="sourceData"/>
    </div>
</template>
  ...
  methods: {
    ...
    onSearch () {
      if (this.searchValue === '') {
        ...
        this.$store.dispatch('setBreadcrumbs', arr)
        // 清除已选节点
        this.$store.dispatch('setSettleApprover', [])
      } else {
    ...
    init (parent) {
      let apiText
      let processId = this.$store.state.handle.processId
      this.roleType = this.$store.state.handle.roleType
      if (this.roleType === 'groupTransactor') {
        apiText = 'groupTransactorSettleApprover'
      } else if (this.roleType === 'groupHost') {
        apiText = 'groupHostSettleApprover'
      }
      handleApi[apiText](
        {
          parent,
